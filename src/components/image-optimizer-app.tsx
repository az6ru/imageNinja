...
export function ImageOptimizerApp() {
  ...
  const displaySavings = totalSavings > 0 && !isNaN(totalSavings) ? (totalSavings / 1024 / 1024).toFixed(1) : '0.0';
  const averageReduction = completedImages.length > 0
    ? Math.round(
        completedImages.reduce((sum, img) => {
          const ratio = typeof img.compressionRatio === 'number' && !isNaN(img.compressionRatio)
